...
/** Public-folder videos (Vite `public/`) — root-relative so URLs work on any route / base. */
const publicVideo = (filename: string) => {
...
const projects = [
  {
    ...
    category: "E-commerce",
    year: "2024",
    video: "clickforcart.mp4",
    description:
      "A shopping web app with product browsing, cart, favorites, and contact flow—built for a smooth retail-style experience and deployed on Netlify.",
    link: "https://clickforcart.netlify.app/",
  },
  {
    title: "Exo-Ape",
    category: "UI Clone",
  ...
  return (
    ...
    >
      <div className="max-w-5xl min-[2000px]:max-w-[70vw] mx-auto">
        ...
        <motion.div
          className="fixed top-0 left-0 w-[min(90vw,28rem)] h-[17.5rem] pointer-events-none z-50 overflow-hidden rounded-xl"
          style={{
            x: imageX,
            y: imageY,
            translateX: "-50%",
        ...
        >
          <div
            className="flex w-full flex-col transition-transform duration-500 ease-[cubic-bezier(0.76,0,0.24,1)]"
            style={{
              height: `${projects.length * 100}%`,
              transform: `translateY(-${((activeProject ?? 0) / projects.length) * 100}%)`,
            }}
          >
